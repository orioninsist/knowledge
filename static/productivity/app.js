...
const focusTotalDuration=()=>state.focus.kind==="pomodoro"?focusDurations()[state.focus.mode]:focusTimerDuration();
const focusProgress=()=>{
    const total=Math.max(1,focusTotalDuration());
    return Math.max(0,Math.min(100,((total-currentFocus())/total)*100));
};
function updateFocusProgress(){
    const bar=$("#focus-progress-bar"),label=$("#focus-progress-label");
    if(!bar||!label)return;
    const value=state.focus.completed?100:focusProgress();
    bar.style.width=value+"%";
    label.textContent=Math.round(value)+"% complete";
}
function focus(){
    ...
    workspace.innerHTML='<div class="focus-shell"><section class="focus-stage"><div class="focus-topline"><div class="focus-unified-modes"><button class="btn '+(state.focus.kind==="pomodoro"&&state.focus.mode==="focus"?"is-active":"")+'" data-focus-preset="focus">Focus</button><button class="btn '+(state.focus.kind==="pomodoro"&&state.focus.mode==="short"?"is-active":"")+'" data-focus-preset="short">Short Break</button><button class="btn '+(state.focus.kind==="pomodoro"&&state.focus.mode==="long"?"is-active":"")+'" data-focus-preset="long">Long Break</button><button class="btn '+(state.focus.kind==="timer"?"is-active":"")+'" data-focus-preset="custom">Custom</button></div><div class="focus-stats"><strong>'+state.focus.sessions+'</strong> sessions · '+Math.round(state.focus.totalSeconds/60)+' min</div></div>'
        +'<div id="focus-timer" class="focus-timer-hero">'+fmt(currentFocus())+'</div>'
        +'<div class="focus-progress-wrap"><div class="progress focus-progress"><span id="focus-progress-bar" style="width:'+(state.focus.completed?100:focusProgress())+'%"></span></div><span id="focus-progress-label" class="focus-progress-label">'+Math.round(state.focus.completed?100:focusProgress())+'% complete</span></div>'
        +status+actions
